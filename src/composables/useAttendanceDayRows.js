...
export function buildDayRows(selectedMonth, logs) {
  ...
  for (const log of logs || []) {
    const d = (typeof log.date === 'string' ? log.date : log.date?.slice?.(0, 10)) ?? ''
    if (d) byDate[d] = log
  }
  ...
  for (let day = 1; day <= lastDay; day++) {
    ...
    const timeIn = log?.time_in != null ? formatTimeAmPm(String(log.time_in)) : null
    const timeOut = log?.time_out != null ? formatTimeAmPm(String(log.time_out)) : null
...
export function totalHoursRenderedInMonth(selectedMonth, logs) {
  if (!selectedMonth || selectedMonth.length < 7 || !Array.isArray(logs)) return 0
  const [y, m] = selectedMonth.split('-').map(Number)
  const pad = (n) => String(n).padStart(2, '0')
  let total = 0
  for (const log of logs) {
